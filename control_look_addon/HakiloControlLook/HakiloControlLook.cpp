...
HakiloControlLook::HakiloControlLook(image_id id)
	: HaikuControlLook()
{
}
...
//	_FillGradient(view, rect, selectedColor, topTint, bottomTint);
	...
	_FillNoGradient(view, rect, selectedColor);
}
	...
	const rgb_color& base, const rgb_color& barColor, float progressPosition)
{
	...
	if (filledSurface) {
		...
		_FillNoGradient(view, filledRect, barColor);
	}
	...
	if (nonfilledSurface) {
		...
		_FillNoGradient(view, nonfilledRect, base);
	}
}
	...
	orientation orientation)
{
	...
	/*_FillGradient(view, barRect, fillColor, fillShadowTint, fillLightTint,
		orientation);
		*/
	_FillNoGradient(view, barRect, fillColor);
}
	...
	const rgb_color& base)
{
	...
	_MakeNoGradient(gradient, rect, base);	
	...
	const rgb_color& base, uint32 flags, orientation orientation) const
{
	...
	if ((flags & B_ACTIVATED) != 0) {
		...
		/*_MakeGlossyGradient(gradient, rect, base, topTint, middleTint1,
			...
			*/
		_MakeNoGradient(gradient, rect, tint_color(base, topTint));	
	}
}
...
{
	return new (std::nothrow)HakiloControlLook(id);
}
